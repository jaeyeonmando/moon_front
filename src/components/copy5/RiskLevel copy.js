...
const RiskLevel = ({ baseScore, impactScore, exploitabilityScore, severity }) => {
  ...
  const severityColors = ['#3498db', '#1abc9c', '#2ecc71', '#a2d729']; // 색상: 파란색, 청록색, 녹색, 연두색
  const severityIndex = severityLevels.indexOf(severity); // 현재 심각도에 해당하는 인덱스
  const severityPercent = (severityIndex + 1) / severityLevels.length; // 게이지 비율
